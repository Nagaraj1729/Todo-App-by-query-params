...
const validateQueryParams = (request, response, next)=> {
    const priorities = ["LOW","low", "MEDIUM","medium", "HIGH","high", undefined]
    const statuses = ["TO DO","to do", "IN PROGRESS","in progress", "DONE","done", undefined]
    const categories = ["WORK","work", "HOME","home", "LEARNING","learning", undefined]
...
app.get("/agenda/", validateQueryDueDate, async (request, response)=>{
    ...
    const getTodoByDueDateQuery = `
        ...
        `
    const todo = await db.get(getTodoByDueDateQuery);
    response.send(todo);
})
